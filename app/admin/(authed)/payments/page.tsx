...
import { updatePayoutAction } from "../actions";
...
async function loadAll() {
  ...
  // 確定済みのみフィルタ
  const ps = ((payouts ?? []) as unknown as (Row & { deal: { status: string } | null })[]).filter(
...
export default async function PaymentsPage() {
  const { list, grand } = await loadAll();
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-8 w-full">
      <header className="mb-6">
        ...
        <p className="text-xs text-gray-500 mt-1">
          各担当者の受取方式（即時／繰延）を反映した、支払うべき金額の一覧。担当者ごとに展開して詳細を確認・更新できます。
        </p>
      </header>
